...
| 核心 | core01 | `172.31.255.11` | Huawei CE12800 V800R023 |
| 汇聚 | agg01-agg02 | `172.31.255.21-22` | FRRouting、OSPF |
| 接入 | access11-access23 | `172.31.255.31-36` | Linux Bridge、VLAN Trunk |
...
./containerlab/scripts/install-containerlab.sh
docker version
...
将合法获得的 CE12800 镜像放入 `containerlab/images/`，文件名必须匹配：

```text
huawei_ce12800-V800R023*.qcow2
```

原始 qcow2、构建目录和校验文件不会提交 Git。进入 WSL2 执行：

```bash
cd /mnt/e/vscode/librenms
./containerlab/scripts/build-ce12800.sh
./containerlab/scripts/deploy.sh
./containerlab/scripts/status.sh
```

启动 LibreNMS 时附加共享管理网络：

```bash
docker compose \
  -f docker/compose.yml \
  -f containerlab/compose.override.yml \
  up -d
```
...
核心至汇聚使用 `10.255.0.0/31` 和 `10.255.0.2/31`，运行 OSPF Area 0。
汇聚 1 承载 VLAN 110、120、130，汇聚 2 承载 VLAN 210、220、230。
